...
// Send lineup email
router.post('/send-lineup', async (req, res) => {
  try {
...
router.get('/email-recipients', async (req, res) => {
  try {
    const result = await query('SELECT * FROM email_recipients');
    ...
    const result = await query(
      `UPDATE email_settings
       SET
         ...
         email_method = $9,
         gmail_client_id = $10,
         gmail_client_secret = $11,
         gmail_refresh_token = $12,
         gmail_redirect_uri = $13,
         gmail_access_token = $14,
         gmail_token_expiry = $15,
         mailgun_api_key = $16,
         mailgun_domain = $17,
         is_eu_region = $18,
         updated_at = NOW()
...
router.post('/email-recipients', async (req, res) => {
  try {
    ...
    const result = await query(
      'INSERT INTO email_recipients (email, created_at) VALUES ($1, NOW()) RETURNING *',
      [email]
    );
    if (result.error) throw result.error;
    res.json({ data: result.data[0] });
...
router.delete('/email-recipients/:id', async (req, res) => {
  try {
    ...
    const result = await query('DELETE FROM email_recipients WHERE id = $1 RETURNING *', [id]);
    if (result.error) throw result.error;
...
router.post('/send-test-email', async (req, res) => {
  try {
    const { showId, testEmail } = req.body;
    ...
    // Get email settings to check the method
    const settingsResult = await query('SELECT * FROM email_settings LIMIT 1');
    ...
    // Get show data - try shows table first, then shows_backup
    ...
    if (!showResult.data || showResult.data.length === 0) {
      return res.status(404).json({ error: 'Show not found in shows or shows_backup tables' });
    }
    ...
    if (emailSettings.email_method === 'mailgun') {
      emailResult = await sendViaMailgun(emailSettings, [testEmail], subject, html);
    } else if (emailSettings.email_method === 'gmail_api') {
      emailResult = await sendViaGmailApi(emailSettings, [testEmail], subject, html);
    } else if (emailSettings.email_method === 'internal_server') {
      emailResult = await sendViaInternalServer(emailSettings, [testEmail], subject, html);
    } else if (emailSettings.email_method === 'smtp') {
      emailResult = await sendViaSmtp(emailSettings, [testEmail], subject, html);
    } else {
      return res.status(400).json({ error: `Unsupported email method: ${emailSettings.email_method}` });
    }
