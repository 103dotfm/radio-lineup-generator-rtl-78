  ...
  return <>
      ...
      return <div key={`bottom-note-${date.toISOString()}`} className="border-b border-r p-2 min-h-[80px] bg-gray-50/50 relative">
            {notesForDate.length > 0 ? <div className="space-y-2">
                {notesForDate.map(note => <div key={note.id} className={`bg-white p-2 rounded shadow-sm cursor-pointer hover:bg-gray-50 transition-colors ${isEditing && note.id === (editingBottomNoteDate as any)?.noteId ? 'ring-2 ring-blue-400' : ''}`}>
                    {isEditing && note.id === (editingBottomNoteDate as any)?.noteId ? <div className="space-y-2">
                      ...
                      </div> : <div onClick={() => handleNoteClick(date, note)} onDoubleClick={() => handleNoteDoubleClick(note.id)} className="text-center bg-violet-200 px-[4px] py-[4px]">
                        {note.note}
                        
                      </div>}
                  </div>)}

                {isEditing && !(editingBottomNoteDate as any).noteId && <div className="bg-white p-2 rounded shadow-sm mt-2">
                ...
                <Button size="sm" variant="ghost" className="mt-2" onClick={() => onBottomNoteAdd(date)} disabled={isEditing}>
                  <Plus className="h-4 w-4 ml-1" /> הערה חדשה
                </Button>
              </div> : isEditing ? <div className="bg-white p-2 rounded shadow-sm">
                  <Textarea value={noteText} onChange={e => setNoteText(e.target.value)} placeholder="הערה..." className="min-h-[60px] text-right" dir="rtl" autoFocus />
                  <div className="flex justify-end space-x-2 space-x-reverse mt-2">
                    <Button size="sm" variant="outline" onClick={handleCancel}>
                      <X className="h-4 w-4 ml-1" /> ביטול
                ...
                </div> : <Button variant="ghost" size="sm" className="flex items-center" onClick={() => onBottomNoteAdd(date)}>
