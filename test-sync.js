...
console.log('Clearing and re-importing...');
...
console.log('Import completed:', result.eventsCreated, 'events');

const todayStr = '2025-12-14';
const todayBookings = await query('SELECT title, start_time, end_time, studio_id FROM studio_bookings WHERE booking_date = $1 AND studio_id = 2 ORDER BY start_time', [todayStr]);
...
expected.forEach(exp => {
  const found = todayBookings.data.find(b => b.start_time === exp.start && b.end_time === exp.end && b.title === exp.title);
  if (!found) {
    missing.push(exp);
  }
});
...
console.log('\n✅ All expected entries present and correct!');
console.log('✅ No problematic entries!');
console.log('✅ Data matches Google Calendar correctly!');
